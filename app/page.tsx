      ...
      {user ? (
        <div className="flex flex-col">
          <span className="scroll-m-20 text-4xl font-extrabold tracking-tight text-center lg:text-5xl text-balance ">
            ...
            <h2 className="bg-accent p-3 py-2 rounded-xl tracking-wider">
              {user?.user_metadata.full_name?.toUpperCase() || user?.email}!
            </h2>
          </span>
        ...
        <div className="flex flex-col px-2 items-center">
          ...
          <p className="leading-7 [&:not(:first-child)]:mt-6 text-center my-4">
            Take control of your tasks and achieve your goals with Task Manager.
            Organize, prioritize, and stay on track with our intuitive app. Get
            started now!
          </p>
          <Link
            href="/auth"
            className={`${buttonVariants({
              variant: "default",
            })}mt-2 flex items-center gap-1 justify-center tracking-wide font-light`}>
            Incia Sesion
          </Link>
        </div>
      )}
